...
## Usage
First, load a dataset and binarize the features.
```python
# load the dataset
dataset_name = 'bank'
...
```
There are two functions that can perform swaps: the recursive and beam search approaches. 
...
**optimize_with_swaps_beam_search**
```python
rs = fasterrisk.RiskScoreOptimizer(X_one_hot, y, k=10, lb=-100, ub=100, gap_tolerance=0.006, select_top_m=-1)
...
Lastly, view each solution and their losses:
```python
